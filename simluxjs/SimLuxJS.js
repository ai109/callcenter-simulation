...
class SimLuxJS {
  ...
  async advance(waitingTime) {
    ...
    let entry = this.#waitingList.find(
      (entry) =>
        entry.awaitedTime == awaitedTime && entry.condition === undefined,
    );
    if (entry === undefined) {
  ...
  #advanceToEntry(targetEntry) {
    ...
    let canAdvanceToTargetTime =
      this.stopTime === undefined || targetTime < this.stopTime;
  ...
  async #continueWithWLEntry(entry) {
    ...
    if (entry.condition !== undefined) {
      for (let [
        controlVariable,
        mapWithUpdateNote,
      ] of this.#controlVariableConditionMap.entries()) {
        let entryQueue = mapWithUpdateNote.conditionEntryMap.get(
          entry.condition,
        );
  ...
  async #waitUntil(controlVariable, conditionFunction, timeout) {
    let mapWithUpdateNote =
      this.#controlVariableConditionMap.get(controlVariable);
    ...
    let entry = new WaitingListEntry(
      conditionFunction,
      timeout !== undefined ? this.#time + timeout : undefined,
    );
  ...
  createControlVariable(value = undefined) {
    return new ControlVariable(
      (controlVariable, condition, timeout) =>
        this.#waitUntil(controlVariable, condition, timeout),
  ...
  async run(until = undefined) {
    ...
    // TW24
    let perf_startTime = performance.now();
    ...
    if (this.#breakPointEntry !== undefined) {
      if (!this.#advanceToEntry(this.#breakPointEntry)) {
        this.#logIfEnabled(
          "Cannot continue because the stop time " +
            this.stopTime +
            " lies before or at the next event time " +
            this.#breakPointEntry.awaitedTime +
            ".",
        );
        return false;
      }
    ...
    do {
      ...
      for (const [
        ...
        for (const [
          condition,
          entryQueue,
        ] of mapWithUpdateNote.conditionEntryMap.entries()) {
          while (
            entryQueue.length > 0 &&
            condition(controlVariable.getValue())
          ) {
            ...
            this.#logIfEnabled(
              entry.waitingSimEntities +
                " SimEntity is resuming because its awaited condition became true.",
            );
            entry.mutex.release();
            ...
            await this.#simulationSemaphore.acquire(
              entry.waitingSimEntities.length,
            );
          }
          if (entryQueue.length == 0) {
      ...
      if (this.#newSimEntities.length > 0) {
        ...
        this.#newSimEntities = [];
        this.#logIfEnabled(
          "Starting " + runningSimEntitys.length + " new SimEntitys.",
        );
        ...
        runningSimEntitys.forEach((SimEntity) =>
          this.#startSimEntity(SimEntity),
        );
      ...
      if (this.#waitingList.length > 0) {
        if (!this.#waitingListIsSorted) {
          this.#waitingList = this.#waitingList.sort(
            (e1, e2) => e2.awaitedTime - e1.awaitedTime,
          );
        ...
        if (!this.#advanceToEntry(nextEntry)) {
          ...
          this.#logIfEnabled(
            "The simulation pauses, because the desired stop time is reached.",
          );
          return false;
        }
...
class SimEntity {
  ...
  constructor(
    runFunction = () => {
      console.info(
        "This SimEntity is doing nothing. Override SimEntity.run or give the SimEntity's constructor a meaningful function to call.",
      );
    },
  ) {
...
/**
 * An entry in the SimLuxJS's waitingList or a conditional entry queue. Either a condition or an awaitedTime must be given.
 */
...
class ControlVariable {
  // These functions are stored here to keep the corresponding SimLuxJS's methods private.
  #waitUntilFunction;
  #newValueFunction;
  #value;
...
module.exports = { SimLuxJS, SimEntity };
